...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        Z1, cache_aff1 = affine_forward(X, W1, b1)
        H1, cache_relu = relu_forward(Z1)
        scores, cache_aff2 = affine_forward(H1, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dH1, dw2, db2 = affine_backward(dy, cache_aff2)
        dZ1 = relu_backward(dH1, cache_relu)
        dx, dw1, db1 = affine_backward(dZ1, cache_aff1)
        loss += 0.5 * self.reg * np.sum(np.square(W1)) \
                + 0.5 * self.reg * np.sum(np.square(W2))
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for index in range(2, self.num_layers):
            self.params['W' + str(index)] = weight_scale * \
                                            np.random.randn(hidden_dims[index-2], hidden_dims[index-1])
            self.params['b' + str(index)] = np.zeros(hidden_dims[index-1])
            # BN
            if use_batchnorm:
                self.params['gamma' + str(index)] = np.ones(hidden_dims[index-1])
                self.params['beta' + str(index)] = np.zeros(hidden_dims[index-1])
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                ...
                if self.use_batchnorm:
                    ...
                    hidden_layers[i + 1], caches[i] = affine_bn_relu_forward(hidden_layers[i], W, b,
